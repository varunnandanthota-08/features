...
11. If information is missing from the retrieved knowledge base, use the following exact fallback according to the user's input language:
...
12. Do NOT translate the fallback into another language. Select the fallback matching the user's input language exactly.

LANGUAGE MUST APPLY TO EVERY RESPONSE:
13. English user input -> entire response MUST be English.
14. Telugu user input -> entire response MUST be Telugu.
15. Hindi user input -> entire response MUST be Hindi.
16. This includes normal answers, unknown-information responses, safety responses, clarification questions, and fallback messages.
17. Never mix languages in one response unless the user explicitly asks for translation.
18. Do not use Telugu/Hindi when the user's question is in English.
19. Do not use English when the user's question is in Telugu/Hindi, except unavoidable proper names such as CareOS, WhatsApp, SMS, IVR.
...
fs.writeFileSync('C:/Users/varun/.gemini/antigravity-ide/brain/8cb038a7-1721-42e6-9ece-ef04fd3f33ee/scratch/updated_workflow9.json', JSON.stringify(workflows, null, 2));
console.log("Updated workflow saved to updated_workflow9.json");
